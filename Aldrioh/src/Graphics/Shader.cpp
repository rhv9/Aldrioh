...
std::pair<bool, GLuint> CreateShader(GLenum shaderType, const char* shaderSource)
{
...
void Shader::Initialize(bool recompileJob)
{
	...
	ASSERT(shaderSources.size() == 2, "Unsupported number of shaders added");

	bool shaderSucceeded = true;
	GLuint shaderIDs[2];
	int shaderPos = 0;
	for (auto&& [type, source] : shaderSources)
	{
		auto [succeededCreateShader, shader] = CreateShader(type, source.c_str());
		shaderIDs[shaderPos++] = shader;
		if (!succeededCreateShader) shaderSucceeded = false;
	}
...
{
	if (!m_Program)
		LOG_CORE_ERROR("Trying to recompile shader when it is not initialized!");
