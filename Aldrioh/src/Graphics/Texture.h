...
public:
	...
	// This constructor is to only wrap a texture object created ourselves so that we can use it.
	Texture(uint32_t textureId, uint32_t width, uint32_t height, int internalFormat, int dataFormat)
		: textureId(textureId), width(width), height(height), internalFormat(internalFormat), dataFormat(dataFormat) {
	}
	~Texture();
